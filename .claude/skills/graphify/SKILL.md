...
- HL7 or FHIR files with populated patient fields (`.hl7`, `.fhir`, FHIR JSON bundles)
- EDI X12 transaction files with real member data (`270`, `271`, `278`, `835`, `837`)
- Database dump/export files from clinical tables
- Application log files from healthcare services (logs may contain PHI from request bodies)
- Files whose names match: `*patient*`, `*member*`, `*phi*`, `*claim*`, `*ehr*`, `*encounter*`, `*beneficiary*`

### Always safe to run on:
- Java/TypeScript/Python source code — architecture and logic only, no PHI
- `src/main/`, `src/test/` trees when test fixtures use synthetic/mock data
- OpenAPI specs, README files, architecture documents
- Configuration files that do not embed real credentials or patient identifiers

### What happens at runtime:
- **Step 1** runs a mandatory PHI pre-flight scan before anything else. If PHI indicators are found, execution stops immediately with a clear message — no extraction, no LLM calls.
- **Step 3B** (semantic extraction) reads file content and sends it to the Claude API. It only runs after the pre-flight scan passes.
- **Outputs** (`graphify-out/graph.json`, `graph.html`, `GRAPH_REPORT.md`) are automatically added to `.gitignore` so they cannot be accidentally committed.
...
### Step 1 - HIPAA pre-flight check + ensure graphify is installed

**Run the PHI pre-flight scan FIRST — before any installation or extraction. If it fails, stop immediately.**

```python
import os, re, sys
from pathlib import Path

INPUT_PATH = "."  # replace with actual path

PHI_FILENAME_PATTERNS = [
    r"patient", r"member", r"phi", r"claim", r"ehr",
    r"encounter", r"beneficiary", r"\.hl7$", r"\.fhir$", r"x12",
]
PHI_EXTENSION_PATTERNS = [r"\.hl7$", r"\.x12$"]
PHI_CONTENT_PATTERNS = [
    r"\bSSN\b", r"\bMRN\b", r"patient[_\s]?id", r"member[_\s]?id",
    r"\bNPI\b.*\d{10}", r"<Patient>", r'"resourceType"\s*:\s*"Patient"',
    r"ISA\*.*GS\*.*ST\*27[0128]",   # EDI X12 transaction sets
]
...
for f in root.rglob("*"):
    ...
    if any(re.search(p, name_lower) for p in PHI_FILENAME_PATTERNS):
        flagged.append(f"Filename match: {f}")
        continue
    # Check content of small text files only (skip binaries and large files)
    if f.suffix.lower() in {".java", ".ts", ".py", ".json", ".xml", ".txt", ".md", ".yml", ".yaml", ".hl7", ".x12"} and f.stat().st_size < 500_000:
        try:
            content = f.read_text(encoding="utf-8", errors="ignore")
            for p in PHI_CONTENT_PATTERNS:
                if re.search(p, content, re.IGNORECASE):
                    flagged.append(f"Content match ({p}): {f}")
                    break
        except Exception:
            pass

if flagged:
    print("HIPAA PRE-FLIGHT FAILED — PHI indicators detected. Extraction stopped.")
    print("Files flagged:")
    ...
    print()
    print("Do NOT run graphify on this path. Use a path that contains only source code and synthetic test data.")
    sys.exit(1)
else:
    print("HIPAA pre-flight passed — no PHI indicators found. Proceeding.")
```

If the pre-flight script exits with code 1, stop immediately. Do not proceed to graphify installation or any extraction step. Tell the user which files were flagged.

Only after a clean pre-flight result, continue:
...
> **HIPAA NOTE:** The subagents below read file content and send it to the Claude API. This step only runs because the pre-flight check in Step 1 confirmed no PHI was detected. If you have any doubt about whether the corpus contains real patient data, stop here and re-run the pre-flight check manually.
...
> **HIPAA NOTE:** Do not push a graph to any shared or externally accessible Neo4j instance if the source codebase belongs to a healthcare service. Even though the pre-flight check passed, graph node labels may contain field names or comments that are considered sensitive. Only push to a local or private internal Neo4j instance.
...
**First — ensure `graphify-out/` is gitignored so outputs are never accidentally committed:**
...
- **HIPAA:** Never skip the pre-flight check, even on `--update` or `--cluster-only` runs.
- **HIPAA:** Never proceed past Step 1 if the pre-flight check flags any file.
- **HIPAA:** Never run `--neo4j-push` against a shared or cloud-hosted Neo4j instance for healthcare service repos.
- **HIPAA:** Always ensure `graphify-out/` is in `.gitignore` before completing Step 9.
